...
autogen_header="""
# THIS FILE WAS GENERATED BY _data/generate.sh
# DO NOT CHANGE THIS FILE, OTHER THAN BY RE-RUNNING THE SCRIPT ABOVE
# (this often needs to happen when source info from devopsdays-web changes)
"""

work_dir=$root_path/temp
mkdir -p $work_dir
...
mkdir -p $root_path/_pages/${YEAR}/speakers/
mkdir -p $root_path/_pages/${YEAR}/sponsors/

DATA_PATH=$root_path/_data/s-$YEAR-sponsors.yaml
echo "$autogen_header\nitems: []" > $DATA_PATH
INDEX=0
...
for ID in $sponsor_ids; do
  ...
  PAGE_PATH=$root_path/_pages/${YEAR}/sponsors/${ID}.markdown
...
DATA_PATH=$root_path/_data/s-$YEAR-speakers.yaml
echo "$autogen_header\nitems: []" > $DATA_PATH
INDEX=0
...
for ID in $person_ids; do
    ...
    PAGE_PATH=$root_path/_pages/${YEAR}/speakers/${ID}.markdown
